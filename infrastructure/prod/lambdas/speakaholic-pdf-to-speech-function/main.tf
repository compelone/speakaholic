terraform {
  backend "s3" {
    ...
    key    = "prod/lambdas/speakaholic-pdf-to-speech-function"
    region = "us-east-1"
  }
}
...
locals {
  ...
  role_arn      = "arn:aws:iam::744137563977:role/production-textract-sns-topic-role"
  sns_topic_arn = "arn:aws:sns:us-east-1:744137563977:production-textract-sns-topic"
...
resource "aws_cloudwatch_log_group" "speakaholic_pdf_to_speech_function" {
  name              = "/aws/lambda/${local.service_name}-${local.vars.environment}"
  retention_in_days = 14
}

# See also the following AWS managed policy: AWSLambdaBasicExecutionRole
resource "aws_iam_policy" "speakaholic_pdf_to_speech_function" {
...
resource "aws_iam_role" "speakaholic_pdf_to_speech_function" {
...
resource "aws_iam_role_policy_attachment" "speakaholic_pdf_to_speech_function" {
...
resource "aws_cloudwatch_metric_alarm" "lambda_error_alarm" {
  ...
  alarm_actions       = [aws_sns_topic.speakaholic_pdf_to_speech_function.arn]

  dimensions = {
    FunctionName = aws_lambda_function.speakaholic_pdf_to_speech_function.function_name
  }
}
...
resource "aws_lambda_permission" "allow_bucket" {
  ...
  function_name = aws_lambda_function.speakaholic_pdf_to_speech_function.arn
